...
{<xsl:for-each select="xmml:memory/gpu:variable"><xsl:text>
    </xsl:text><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/>;    /**&lt; X-machine memory variable <xsl:value-of select="xmml:name"/> of type <xsl:value-of select="xmml:type"/>.*/</xsl:for-each>
};
</xsl:for-each>
...
{	
    ...
    <xsl:for-each select="xmml:memory/gpu:variable"><xsl:text>
    </xsl:text><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/> [xmachine_memory_<xsl:value-of select="../../xmml:name"/>_MAX];    /**&lt; X-machine memory variable list <xsl:value-of select="xmml:name"/> of type <xsl:value-of select="xmml:type"/>.*/</xsl:for-each>
};
</xsl:for-each>
...
struct xmachine_message_<xsl:value-of select="xmml:name"/>_PBM
{
	...
	int end[xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size];
};
</xsl:if></xsl:for-each>
 ...
 * @param agents	xmachine_memory_<xsl:value-of select="xmml:name"/>_list agent list
 ...
 __FLAME_GPU_FUNC__ void add_<xsl:value-of select="xmml:name"/>_message(xmachine_message_<xsl:value-of select="xmml:name"/>_list* <xsl:value-of select="xmml:name"/>_messages, <xsl:for-each select="xmml:variables/gpu:variable"><?foreach messagevar?><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>);
...
/** add_<xsl:value-of select="xmml:name"/>_agent
 * Adds a new continuous valued <xsl:value-of select="xmml:name"/> agent to the xmachine_memory_<xsl:value-of select="xmml:name"/>_list list using a linear mapping
 * @param agents xmachine_memory_<xsl:value-of select="xmml:name"/>_list agent list
 <xsl:for-each select="xmml:memory/gpu:variable">* @param <xsl:value-of select="xmml:name"/>	agent agent variable of type <xsl:value-of select="xmml:type"/><xsl:text>
 </xsl:text></xsl:for-each>*/
__FLAME_GPU_FUNC__ void add_<xsl:value-of select="xmml:name"/>_agent(xmachine_memory_<xsl:value-of select="xmml:name"/>_list* agents, <xsl:for-each select="xmml:memory/gpu:variable"><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>);
</xsl:if></xsl:for-each>
...
extern "C" void set_<xsl:value-of select="xmml:name"/>(<xsl:value-of select="xmml:type"/>* h_<xsl:value-of select="xmml:name"/>);
  ...
  */
  ...
  extern "C" void setSimulationDescription(char * simulationDescription);
  #endif
  //#ifdef FLAME_GPU_EXTENSIONS
  /* FLAME GPU EXTENSIONS SECTION */
