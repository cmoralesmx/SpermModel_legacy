...
<!--Recursive template for function recursive condititions-->
<xsl:template match="gpu:recursiveCondition">(<xsl:choose>
<xsl:when test="xmml:lhs/xmml:value"><xsl:value-of select="xmml:lhs/xmml:value"/>
</xsl:when>
<xsl:when test="xmml:lhs/xmml:agentVariable">currentState-><xsl:value-of select="xmml:lhs/xmml:agentVariable"/>[index]</xsl:when>
<xsl:otherwise><xsl:apply-templates select="xmml:lhs/xmml:condition"/>
...
texture&lt;int, 1, cudaReadModeElementType&gt; tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end;
__constant__ int d_tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end_offset;
 ...
 */
 __global__ void <xsl:value-of select="../xmml:name"/>_function_filter(xmachine_memory_<xsl:value-of select="../../../xmml:name"/>_list* currentState)
 {
  ...
  /* FLAME GPU EXTENSIONS SECTION */
  <xsl:for-each select="gpu:xmodel/xmml:xagents/gpu:xagent/xmml:functions/gpu:function/gpu:recursiveCondition">
    /** <xsl:value-of select="../xmml:name"/>_function_filter
    *	Global condition function. Flags the scan input state to true if the condition is met
    * @param currentState xmachine_memory_<xsl:value-of select="../../../xmml:name"/>_list representing agent i the current state
    */
...
__global__ void scatter_<xsl:value-of select="xmml:name"/>_Agents(xmachine_memory_<xsl:value-of select="xmml:name"/>_list* agents_dst, xmachine_memory_<xsl:value-of select="xmml:name"/>_list* agents_src, int dst_agent_count, int number_to_scatter){
	...
	if ((_scan_input == 1)&amp;&amp;(index &lt; number_to_scatter)){
		...
		agents_dst->_position[output_index] = output_index;<xsl:for-each select="xmml:memory/gpu:variable">
		agents_dst-><xsl:value-of select="xmml:name"/>[output_index] = agents_src-><xsl:value-of select="xmml:name"/>[index];</xsl:for-each>
	}
}
...
__device__ void add_<xsl:value-of select="xmml:name"/>_agent(xmachine_memory_<xsl:value-of select="xmml:name"/>_list* agents, <xsl:for-each select="xmml:memory/gpu:variable"><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>){
	...
	//write data to new buffer<xsl:for-each select="xmml:memory/gpu:variable">
	agents-><xsl:value-of select="xmml:name"/>[index] = <xsl:value-of select="xmml:name"/>;</xsl:for-each>

}

//non templated version assumes DISCRETE_2D but works also for CONTINUOUS
__device__ void add_<xsl:value-of select="xmml:name"/>_agent(xmachine_memory_<xsl:value-of select="xmml:name"/>_list* agents, <xsl:for-each select="xmml:memory/gpu:variable"><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>){
    add_<xsl:value-of select="xmml:name"/>_agent&lt;DISCRETE_2D&gt;(agents, <xsl:for-each select="xmml:memory/gpu:variable"><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>);
}
...
__global__ void reorder_<xsl:value-of select="xmml:name"/>_agents(unsigned int* values, xmachine_memory_<xsl:value-of select="xmml:name"/>_list* unordered_agents, xmachine_memory_<xsl:value-of select="xmml:name"/>_list* ordered_agents)
{
	...
	//reorder agent data<xsl:for-each select="xmml:memory/gpu:variable">
	ordered_agents-><xsl:value-of select="xmml:name"/>[index] = unordered_agents-><xsl:value-of select="xmml:name"/>[old_pos];</xsl:for-each>
}
</xsl:if></xsl:for-each>
...
__device__ void add_<xsl:value-of select="xmml:name"/>_message(xmachine_message_<xsl:value-of select="xmml:name"/>_list* messages, <xsl:for-each select="xmml:variables/gpu:variable"><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>){
	...
	//global thread index
	int index = __mul24(blockIdx.x,blockDim.x) + threadIdx.x + d_message_<xsl:value-of select="xmml:name"/>_count;
...
__device__ xmachine_message_<xsl:value-of select="xmml:name"/>* get_first_<xsl:value-of select="xmml:name"/>_message(xmachine_message_<xsl:value-of select="xmml:name"/>_list* messages){
	...
	int wrap_size = __mul24(ceil((float)d_message_<xsl:value-of select="xmml:name"/>_count/ blockDim.x), blockDim.x);
...
__device__ void add_<xsl:value-of select="xmml:name"/>_message(xmachine_message_<xsl:value-of select="xmml:name"/>_list* messages, <xsl:for-each select="xmml:variables/gpu:variable"><xsl:value-of select="xmml:type"/><xsl:text> </xsl:text><xsl:value-of select="xmml:name"/><xsl:if test="position()!=last()">, </xsl:if></xsl:for-each>){
	if (AGENT_TYPE == DISCRETE_2D){
		...
		int index = global_position.x + __mul24(global_position.y, width);
...
__device__ xmachine_message_<xsl:value-of select="xmml:name"/>* get_first_<xsl:value-of select="xmml:name"/>_message_continuous(xmachine_message_<xsl:value-of select="xmml:name"/>_list* messages,  int agent_x, int agent_y){
	...
	int index = __mul24((global_position.y), width) + global_position.x;
...
__device__ xmachine_message_<xsl:value-of select="xmml:name"/>* get_first_<xsl:value-of select="xmml:name"/>_message_discrete(xmachine_message_<xsl:value-of select="xmml:name"/>_list* messages){
	...
	int range = d_message_<xsl:value-of select="xmml:name"/>_range;
	int width = d_message_<xsl:value-of select="xmml:name"/>_width;
	int sm_grid_width = blockDim.x + __mul24(range, 2);
	...
	int sm_index = __mul24(sm_pos.y, sm_grid_width) + sm_pos.x;
	...
	if (left_border){	
		...
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24(sm_pos.y, sm_grid_width) + threadIdx.x;
	...
	if (right_border){
		...
		border_index_2d.x = sWRAP(border_index_2d.x + range, width);
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24(sm_pos.y, sm_grid_width) + (sm_pos.x + range);
	...
	if (top_border){
		...
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24(threadIdx.y, sm_grid_width) + sm_pos.x;
	...
	if (bottom_border){
		...
		border_index_2d.y = sWRAP(border_index_2d.y + range, width);
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24((sm_pos.y + range), sm_grid_width) + sm_pos.x;
	...
	if ((top_border)&amp;&amp;(left_border)){	
		...
		border_index_2d.y = sWRAP(border_index_2d.y - range, width);
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24(threadIdx.y, sm_grid_width) + threadIdx.x;
	...
	if ((top_border)&amp;&amp;(right_border)){	
		...
		border_index_2d.y = sWRAP(border_index_2d.y - range, width);
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24(threadIdx.y, sm_grid_width) + (sm_pos.x + range);
	...
	if ((bottom_border)&amp;&amp;(right_border)){	
		...
		border_index_2d.y = sWRAP(border_index_2d.y + range, width);
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24((sm_pos.y + range), sm_grid_width) + (sm_pos.x + range);
	...
	if ((bottom_border)&amp;&amp;(left_border)){	
		...
		border_index_2d.y = sWRAP(border_index_2d.y + range, width);
		border_index = __mul24(border_index_2d.y, width) + border_index_2d.x;
		sm_border_index = __mul24((sm_pos.y + range), sm_grid_width) + threadIdx.x;
	...
	sm_index = __mul24(threadIdx.y, sm_grid_width) + threadIdx.x;
...
__device__ unsigned int message_<xsl:value-of select="xmml:name"/>_hash(int3 gridPos)
{
	...
	//unique id
	return __umul24(__umul24(gridPos.z, d_message_<xsl:value-of select="xmml:name"/>_partitionDim.y), d_message_<xsl:value-of select="xmml:name"/>_partitionDim.x) + __umul24(gridPos.y, d_message_<xsl:value-of select="xmml:name"/>_partitionDim.x) + gridPos.x;
}
	...
	while(move_cell)
	{
        ...
        if (next_cell<xsl:choose><xsl:when test="ceiling((gpu:partitioningSpatial/gpu:zmax - gpu:partitioningSpatial/gpu:zmin) div gpu:partitioningSpatial/gpu:radius) = 1">2D</xsl:when><xsl:otherwise>3D</xsl:otherwise></xsl:choose>(&amp;relative_cell))
		{
			...
			if (cell_index_min != 0xffffffff)
			{
				...
				cell_index_max = tex1Dfetch(tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end, next_cell_hash + d_tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end_offset);
...
__global__ void GPUFLAME_<xsl:value-of select="xmml:name"/>(xmachine_memory_<xsl:value-of select="../../xmml:name"/>_list* agents<xsl:if test="xmml:xagentOutputs/gpu:xagentOutput">, xmachine_memory_<xsl:value-of select="xmml:xagentOutputs/gpu:xagentOutput/xmml:xagentName"/>_list* <xsl:value-of select="xmml:xagentOutputs/gpu:xagentOutput/xmml:xagentName"/>_agents</xsl:if>
    ...
    <!-- Can only use this for non brute force messaging as brute force messaging requires full threa blcok sizes -->
	...
	//SoA to AoS - xmachine_memory_<xsl:value-of select="xmml:name"/> Coalesced memory read
	xmachine_memory_<xsl:value-of select="../../xmml:name"/> agent;<xsl:for-each select="../../xmml:memory/gpu:variable">
	agent.<xsl:value-of select="xmml:name"/> = agents-><xsl:value-of select="xmml:name"/>[index];</xsl:for-each>
	...
	//AoS to SoA - xmachine_memory_<xsl:value-of select="xmml:name"/> Coalesced memory write<xsl:for-each select="../../xmml:memory/gpu:variable">
	agents-><xsl:value-of select="xmml:name"/>[index] = agent.<xsl:value-of select="xmml:name"/>;</xsl:for-each>
}
</xsl:for-each>
